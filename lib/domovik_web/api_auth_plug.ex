defmodule DomovikWeb.APIAuthPlug do
  @moduledoc """
  This module contains all functions related to gatekeeping the API access.
  """
  use Pow.Plug.Base
  ...
  @doc """
  Fetches the user from an access token
  """
  @impl true
  ...
  The tokens are added to the `conn.private` as `:api_access_token` and `:api_renewal_token`.
  The renewal token is stored in the access token metadata, and vice-versa.
  """
  @impl true
  ...
  The renewal token is deleted by fetchin it from the access token metadata.
  """
  @impl true
  ...
  The access token, if any, will be deleted by fetching it from the renewal token metatada.
  The renewal token will be deleted from the store after it has been fetched.
  """
